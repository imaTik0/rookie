...
export class MigrationManager {
  ...
  constructor(
    ...
    this.migrationsCollection = this.mongoDbConnection.getCollection(this.migrationsCollectionName);
  }

  async initialize(): Promise<void> {
    const collections = await this.mongoDbConnection.getDb().listCollections({ migrationName: this.migrationsCollectionName }).toArray();
    if (collections.length === 0) {
      await this.mongoDbConnection.getDb().createCollection(this.migrationsCollectionName);
      this.logger.log("Migrations collection created");
    }
  }

  async runMigration(migration: Migration): Promise<void> { // For CLI/Client later? 
    const isMigrated = await this.migrationsCollection.findOne({ migrationName: migration.migrationName });
    if (isMigrated) {
      this.logger.log(`Migration ${migration.migrationName} has already been applied.`);
      return;
    }

    this.logger.log(`Running migration: ${migration.migrationName}`);
    await migration.up(this.mongoDbConnection);
    await this.migrationsCollection.insertOne({ migrationName: migration.migrationName, date: new Date() });
  ...
  async rollbackMigration(migration: Migration): Promise<void> { // For CLI/Client later? 
    const isMigrated = await this.migrationsCollection.findOne({ migrationName: migration.migrationName });
    if (!isMigrated) {
      this.logger.log(`Migration ${migration.migrationName} has not been applied, so it cannot be rolled back.`);
      return;
    }

    this.logger.log(`Rolling back migration: ${migration.migrationName}`);
    await migration.down(this.mongoDbConnection);
    await this.migrationsCollection.deleteOne({ migrationName: migration.migrationName });
