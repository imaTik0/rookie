...
export default function ExecutionConsole() {
  ...
  const handleExecute = () => {
    ...
    const eventSource = new EventSource(`http://localhost:3000/testsuites/${suiteId}/execute/stream`);
      ...
      try {
        ...
        setLogs(prev => {
          ...
          if (parsed.type === 'token' && lastLog && lastLog.type === 'token') {
            ...
            newLogs[lastIndex] = { ...lastLog, content: lastLog.content + parsed.content };
            return newLogs;
          }
    ...
    eventSource.addEventListener('error', (event: any) => {
      if (event.data) {
        setLogs(prev => [...prev, { type: 'log', content: `[ERROR FATAL]: ${event.data}` }]);
      }
      eventSource.close();
      setIsRunning(false);
    });
