...
const c = isTTY
    ...
    : Object.fromEntries(
        ["R", "b", "d", "rd", "gr", "ye", "bl", "mg", "cy", "wh", "gy"].map((k) => [k, ""]),
...
function printOverview(r: Report): void {
    ...
    console.log(
        `  ${buildBar(bRate, 30)}  ${bold(`${bPass}/${bTotal}`)} passed  ${gray(`(${bRate}%)`)}`,
    );
    console.log(`\n  ${bold("Experiment")} (old docs + new API)`);
    console.log(
        `  ${buildBar(eRate, 30)}  ${bold(`${ePass}/${eTotal}`)} passed  ${
            eRate < bRate ? red(`(${eRate}%)`) : green(`(${eRate}%)`)
        }`,
    );
    ...
    if (d.regressions.length === 0 && d.improvements.length === 0) {
        ...
        if (d.regressions.length > 0) {
            console.log(
                `  ${red("⚠")}  ${
                    bold(red(`${d.regressions.length} regression(s)`))
                } — goals that PASSED on old API but FAILED on new API`,
            );
        }
        if (d.improvements.length > 0) {
            console.log(
                `  ${green("↑")}  ${
                    bold(green(`${d.improvements.length} improvement(s)`))
                } — goals that FAILED on old API but PASSED on new API`,
            );
        }
    }
}
...
function printFailureTaxonomy(r: Report): void {
    ...
    console.log(
        tableRow(
            cell(bold("Error class"), WC),
            cell(bold("Baseline"), WB),
            cell(bold("Experiment"), WE),
        ),
    );
    console.log(tableDivider(widths, "═"));
    ...
    for (const cls of classes) {
        ...
        const eBar = buildInlineBar(eCnt, maxCount, BAR_W, eCnt > bCnt ? c.rd : c.gr) + " " +
            String(eCnt).padStart(2);
    ...
    gaps.forEach((gap, i) => {
        ...
        if (gap.file) {
            const loc = gap.lineStart
                ? `${gap.file}:${gap.lineStart}${
                    gap.lineEnd && gap.lineEnd !== gap.lineStart ? `–${gap.lineEnd}` : ""
                }`
                : gap.file;
...
function printDocsPatch(r: Report): void {
    ...
    console.log(
        `  ${p.patchedClusters} patchable cluster(s), ${p.unpatchedClusters} suggestion(s) without verified location`,
    );
    if (p.patchFile) {
        console.log(
            `  ${green("✓")} unified diff:  ${cyan(p.patchFile)}  ${dim("(git apply-able)")}`,
        );
    }
...
function printTopFailingFunctions(r: Report): void {
    ...
    console.log(tableTop(widths));
    console.log(
        tableRow(cell(bold("Function"), WF), cell(bold("Count"), WN), cell(bold("Goals"), WG)),
    );
    console.log(tableDivider(widths, "═"));
...
function main(): void {
    ...
    if (!filePath || filePath.startsWith("--")) {
        console.error(
            "Usage: deno run --allow-read scripts/print-report.ts <report.json> [--goals] [--gaps] [--full]",
        );
        Deno.exit(1);
    }
