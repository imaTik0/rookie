...
const c = isTTY
  ? { R:"\x1b[0m", b:"\x1b[1m", d:"\x1b[2m", rd:"\x1b[31m", gr:"\x1b[32m",
      ye:"\x1b[33m", bl:"\x1b[34m", mg:"\x1b[35m", cy:"\x1b[36m", gy:"\x1b[90m" }
  : Object.fromEntries(["R","b","d","rd","gr","ye","bl","mg","cy","gy"].map(k => [k, ""])) as Record<string,string>;
...
//  CLI ARGS
// ─────────────────────────────────────────────────────────────────
const arg  = (k: string): string | null => { const i = Deno.args.indexOf(k); return i !== -1 ? Deno.args[i + 1] ?? null : null; };
...
if (!cfg) {
  console.error(`Unknown config "${configName}". Available: ${Object.keys(EXPERIMENTS).join(", ")}`);
  Deno.exit(1);
}
...
function banner(title: string, width = 66): void {
  ...
  console.log(`\n${bold(line)}`);
  console.log(bold(`${"═".repeat(pad)} ${title} ${"═".repeat(Math.max(0, width - pad - title.length - 2))}`));
...
async function dockerStop(name: string): Promise<void> {
  await new Deno.Command("docker", { args: ["stop", name], stdout: "null", stderr: "null" }).output().catch(() => {});
  await new Deno.Command("docker", { args: ["rm",   name], stdout: "null", stderr: "null" }).output().catch(() => {});
}
...
async function dockerStart(image: string, container: ContainerConfig): Promise<void> {
  ...
  const args = ["run", "-d", "--name", container.name, "-p", `${container.hostPort}:${container.port}`, ...envArgs, image];
...
function renderSchemaProps(
  ...
  for (const [field, fieldSchema] of Object.entries(properties)) {
    ...
    const type = (fs.type ?? fs.format ?? (fs.$ref ? (fs.$ref as string).split("/").pop() : "object")) as string;
...
function swaggerToMarkdown(spec: Record<string, unknown>): string {
  ...
  const byTag = new Map<string, Array<{ method: string; path: string; op: Record<string, unknown> }>>();
    ...
    for (const { method, path, op } of ops) {
      ...
      if (nonBody.length) {
        ...
        for (const p of nonBody) {
          ...
          const type = (p.schema as Record<string,unknown> | undefined)?.type ?? p.type ?? "?";
          lines.push(`- \`${p.name}\` (${p.in}) ${req} \`${type}\` — ${p.description ?? ""}`);
        }
      }

      // ── Swagger 2.0 body parameter (uses $ref into definitions) ─
      const bodyParam = params.find(p => p.in === "body") as Record<string, unknown> | undefined;
      if (bodyParam?.schema) {
        const schemaNode = bodyParam.schema as Record<string, unknown>;
        const refName    = typeof schemaNode.$ref === "string" ? schemaNode.$ref.split("/").pop() : undefined;
      ...
      if (reqBody?.content) {
        ...
        for (const [ct, media] of Object.entries(reqBody.content as Record<string, Record<string, unknown>>)) {
          lines.push(`- Content-Type: \`${ct}\``);
          if (media.schema) renderSchemaProps(spec, media.schema as Record<string, unknown>, lines, "    ");
        }
      }
...
async function ingestDocs(docsCfg: DocsConfig, image: string, vars: Record<string, unknown>): Promise<{ id: string; files?: unknown[] }> {
  ...
  if (docsCfg.mode === "swagger-json") {
    ...
    console.log(`${gray("│")}  ${yell("▸")} converted to Markdown (${chars} chars, ~${Math.round(chars / 4)} tokens)`);
...
async function* ndJsonStream(url: string, body: unknown): AsyncGenerator<Record<string, unknown>> {
  ...
  while (true) {
    ...
    for (const line of lines) {
      const t = line.trim();
      if (t) { try { yield JSON.parse(t); } catch { /* skip malformed */ } }
    }
  }
  if (buf.trim()) { try { yield JSON.parse(buf.trim()); } catch { /* trailing data */ } }
}
  ...
  for await (const event of ndJsonStream(`${ROOKIE_URL}/planner/run`, { projectId, maxGoals: plannerCfg.maxGoals, initialContext: context })) {
    switch (event.type) {
      ...
      case "SUMMARY_GENERATED":
        ...
        String(event.summary ?? "").split("\n").slice(0, 8).forEach(l => console.log(`${gray("│")}    ${l}`));
        break;

      case "COMPLETE":
        masterPlan = event.result as Record<string, unknown>;
        console.log(`${gray("│")}`);
        console.log(`${gray("│")}  ${green("✓")} master plan complete  ${(event.result as Record<string,unknown>)?._id ? gray(String((event.result as Record<string,unknown>)._id)) : ""}`);
        break;
...
function analyzeDrift(baselineBreakdown: GoalResult[], experimentBreakdown: GoalResult[]) {
  ...
  const regressions:  Array<{ goal: string; baseline: string; experiment: string; reportId: string | null }> = [];
...
function printFinalSummary(report: Record<string, unknown>): void {
  ...
  if (drift.regressions.length === 0) {
    ...
    console.log(`\n  ${red("⚠")}  ${bold(red(`${drift.regressions.length} regression(s)`))} — documentation drift detected:`);
...
async function setupGiteaAdmin(containerName: string): Promise<Record<string, string>> {
  ...
  if (code !== 0) {
    throw new Error(`Gitea token generation failed: ${new TextDecoder().decode(stderr).trim()}`);
  }
...
async function main(): Promise<void> {
  ...
  const externalDocs    = !resolvedDocsUrl.includes("localhost") && !resolvedDocsUrl.includes("127.0.0.1");
  ...
  const { masterPlan: bPlan, goals: bGoals, breakdown: bBreakdown } =
  ...
  const report = {
    ...
    baseline: {
      ...
      markdownSummary:   ((bPlan as Record<string,unknown> | null)?.detailedResults as Record<string,unknown> | undefined)?.finalOutput ?? null,
    },
    experiment: {
  ...
  console.log(`  Run ${bold(cyan(`deno run --allow-all scripts/print-report.ts ${outFile}`))} for a detailed view.\n`);
}
